...
class Axs_CheckoutFailModuleFrontController extends ModuleFrontController
{
    public function postProcess()
    {
        ...
        PrestaShopLogger::addLog(
            'AXS FAIL LOG: ' . print_r($_REQUEST, true),
            1,
            null,
            'Axs_Checkout',
            0,
        ...
        if ($order && Validate::isLoadedObject($order) && $normalizedStatus === 'pending') {
            ...
            if ($normalizedStatus === 'pending') {
                $module->applyPaymentStatusToOrder($order, 'failed', [
                    'transactionRef' => (string) Tools::getValue('transactionRef'),
                    'amount' => (string) Tools::getValue('amount'),
                    'currency' => (string) Tools::getValue('currency'),
                ]);
        ...
        if ($order && Validate::isLoadedObject($order) && !$module->restoreRetryCartFromOrder($order)) {
            PrestaShopLogger::addLog(
                sprintf('AXS Checkout: failed to restore retry cart from order %d after fail redirect', (int) $order->id),
                3,
                null,
                'Order',
                (int) $order->id,
        ...
        $this->errors[] = $this->getFailureMessage($module, $normalizedStatus);
    ...
    private function getFailureMessage(Axs_Checkout $module, $normalizedStatus)
    {
        switch ($normalizedStatus) {
            case 'canceled':
                return $module->l('Payment Cancelled. You have not been charged.', 'fail');

            case 'failed':
                return $module->l('Payment Failed. You can try again using a different payment method.', 'fail');
